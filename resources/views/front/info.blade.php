    ...
    <div class="leftbox">
        <div class="infos">
            <div class="newsview">
                ...
                <div class="news_author"><span class="au01">Shadow</span><span class="au02">{{$data['current']['create_at']}}</span><span class="au03">共<b>{{$data['current']['browse']}}</b>人围观</span></div>
                <div class="tags">
                    @foreach($data['current']['tags'] as $value)
                        <a href="javascript:void(0);">{{$value}}</a>
                    @endforeach
                </div>
                <div class="news_about"><strong>简介</strong>{!! $data['current']['introduction']!!}</div>
                <div class="news_infos">
                    {!!$data['current']['content']!!}
                </div>
            </div>
            <div class="share">
                <p class="diggit"><a href="javascript:void(0)" data-id="{{$data['current']['id']}}"> 很赞哦！ </a>(<b id="diggnum">{{$data['current']['love']}}</b>)</p>
        ...
        <div class="nextinfo">
            <p>上一篇：<a href="/front/info?id={{$data['prev']['id']}}" >{{$data['prev']['title']}}</a></p>
            <p>下一篇：<a href="/front/info?id={{$data['next']['id']}}" >{{$data['next']['title']}}</a></p>
        </div>
        <div class="otherlink">
            <h2>相关文章</h2>
            <ul>
                @foreach($data['recommend'] as $value)
                    <li><a href="/front/info?id={{$value['id']}}">{{$value['title']}}</a></li>
                @endforeach
            </ul>
        </div>
        <div class="news_pl">
