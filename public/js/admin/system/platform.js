layui.use(['form', 'layedit', 'laydate','element','jquery','layer','upload'], function() {
     ...
     * Editor rule
     */
    $('.develop_rule_editor').on('click',function () {
        var name = $(this).parents('h3').children('.name').text();
        ...
        template = '<form class="layui-form">\n' +
            ...
            '      <input type="text" name="name" required value="'+ name +'" placeholder="名称" autocomplete="off" class="layui-input">\n' +
            ...
            '  <div class="layui-form-item">\n' +
            '    <div class="layui-input-block">\n' +
            '      <button class="layui-btn" lay-submit lay-filter="rules">确定</button>\n' +
            '    </div>\n' +
            '  </div>\n' +
            '</form>';
        index = layer.open({
     ...
     * 删除权限
     */
    $('.develop_rule_del').on('click',function () {
        var id = $(this).attr("data-id");
